...
func NewTestConsoleWithWriter(w io.Writer) *Logger {
	zerolog.TimeFieldFormat = ""
	zerolog.TimestampFunc = func() time.Time {
		return time.Date(2008, 1, 8, 17, 5, 5, 0, time.UTC)
	}
...
func (l *Logger) ReqLogFromRequest(r *http.Request) []byte {
	return []byte(fmt.Sprintf(fmtReqLogFromRequest, r.Method, r.URL.RequestURI(), r.ProtoMajor, r.ProtoMinor, r.Host))
}
